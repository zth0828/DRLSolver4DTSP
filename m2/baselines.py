...
class RolloutBaseline(Baseline):
    ...
    def __init__(self, mat, ci, model, opts, model1, model2, epoch=0):
        super(Baseline, self).__init__()
        self.mat = mat
        self.opts = opts
        self.ci = ci
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        self.bl_vals = rollout(self.mat, self.model, self.dataset, self.opts, self.model1, self.model2).cpu().numpy()
    ...
    def wrap_dataset(self, dataset):
        ...
        return BaselineDataset(dataset, rollout(self.mat, self.model, dataset, self.opts, self.model1, self.model2).view(-1, 1))
    ...
    def epoch_callback(self, model, epoch):
        ...
        candidate_vals = rollout(self.mat, model, self.dataset, self.opts, self.model1, self.model2).cpu().numpy()
